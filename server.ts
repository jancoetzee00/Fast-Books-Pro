...
// AI-based Expense Classification Helper
app.post("/api/gemini/classify-expense", async (req, res) => {
  try {
    const { description, vendor, amount } = req.body;
    const ai = getGeminiClient();
    ...
    const systemInstruction = `You are the AI Expense Classification Engine for Fast-Books PRO (South African bookkeeping system for Jan Coetzee).
Analyze the expense description, vendor name, and amount, and classify it into the most accurate business category.

Valid Categories MUST be exactly one of:
- "Office Supplies" (e.g. stationery, printer paper, ink cartridges, packaging, pens, notebooks, office consumables)
- "Utilities" (e.g. Eskom electricity, municipal water, City of Joburg/Cape Town/Tshwane, fibre internet, broadband, Vodacom, MTN, Telkom, waste/refuse)
- "Travel" (e.g. Uber, Bolt, FlySafair, Airlink, flights, hotel, accommodation, Airbnb, Avis/Hertz car rental, parking, Gautrain, toll gates)
- "Vehicle & Petrol" (e.g. petrol, diesel, fuel, Engen, Shell, BP, Caltex, Sasol, Total, vehicle repairs, tyres, oil service, mechanic)
- "Software & Cloud" (e.g. Google Workspace, Microsoft 365, AWS, GitHub, Figma, Adobe, Zoom, OpenAI, domains, cloud hosting)
- "Rent & Property" (e.g. office rent, workshop lease, building levies, storage unit, commercial security)
- "Professional Fees" (e.g. legal attorney fees, SAIPA/SAICA audit, tax consulting, compliance certifications)
- "Marketing & Ads" (e.g. Google Ads, Facebook/Meta Ads, LinkedIn Ads, graphic design, print flyers, billboard, SEO)
- "Operating Expenses" (e.g. staff tea/coffee, cleaning materials, small workshop tools, daily general supplies)
- "Other" (e.g. bank charges, interest, miscellaneous)

Respond ONLY with a JSON object in this format:
{
  "category": "Office Supplies" | "Utilities" | "Travel" | "Vehicle & Petrol" | "Software & Cloud" | "Rent & Property" | "Professional Fees" | "Marketing & Ads" | "Operating Expenses" | "Other",
  "confidence": 0.95,
  "reason": "Clear 1-sentence explanation why this category matches South African accounting practices",
  "suggestedVendor": "Clean formatted vendor/payee name",
  "taxDeductible": true,
  "taxDeductibleReason": "Standard business expense deductible under SARS Section 11(a)"
}`;

    const response = await ai.models.generateContent({
      model: "gemini-3.7-flash",
      contents: `Classify this business expense:
...
Amount: R ${amount || 0}`,
    ...
    try {
      ...
      res.json({
        ...
        category: "Operating Expenses",
        confidence: 0.7,
        reason: "General operational expense",
      });
    }
  } catch (error: any) {
    console.error("Error in /api/gemini/classify-expense:", error);
    res.status(500).json({ error: error.message || "Expense classification error" });
  }
});
